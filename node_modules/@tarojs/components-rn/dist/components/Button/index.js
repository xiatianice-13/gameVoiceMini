...
const Loading = (props) => {
    ...
    React.useEffect(() => {
        const animation = Animated.loop(Animated.timing(rotate, {
            toValue: 1,
            duration: 1000,
    ...
    const loadingStyle = Object.assign(Object.assign({}, styles.loading), { transform: [{ rotate: rotateDeg }] });
    if (!hasSibling) {
        loadingStyle.marginRight = 0;
    }
    return (React.createElement(Animated.View, { testID: 'loading', style: loadingStyle },
        React.createElement(Image, { accessibilityLabel: 'loading image', source: type === 'warn' ? loadingWarnPng : ladingPng, style: styles.loadingImg })));
};
class _Button extends React.Component {
    ...
    render() {
        ...
        return (React.createElement(TouchableWithoutFeedback, { onPress: this.onPress, onLongPress: this.onPress, onPressIn: this.onPressIn, onPressOut: this.onPressOut, ref: this.$touchable, accessibilityRole: "button", disabled: disabled },
            React.createElement(View, { style: [
                ...
                loading && React.createElement(Loading, { hasSibling: !!React.Children.count(children), type: type }),
                Array.isArray(children) ? (children.map((c, i) => (React.createElement(Text, { key: i, style: [styles.btnText, !isDefaultSize && styles.btnTextMini, { color: textColor }, textHoverStyle] }, c)))) : (['string', 'number'].indexOf(typeof children) > -1) ? (React.createElement(Text, { style: [styles.btnText, !isDefaultSize && styles.btnTextMini, { color: textColor }, textHoverStyle] }, children)) : (children))));
    }
}
